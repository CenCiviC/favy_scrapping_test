...
class ShoppingMallInfoExtractor: 
    def __init__(self, url): 
        ...
        if not self.scraper.check_response_status():
            print(f"Error in {url}")
            return
        
        self.consider_shoppingmall()
    ...
    def consider_shoppingmall(self):
        self.info.siteUrl = self.scraper.url
        if self.info.siteUrl.find('zigzag') != -1:
            self.extract_zigag_info()
        elif self.info.siteUrl.find('a-bly') != -1:
            self.extract_ably_info()
        elif self.info.siteUrl.find('wconcept') != -1:
            self.extract_wconcept_info()
        elif self.info.siteUrl.find('musinsa') != -1:
            self.extract_musinsa_info()
        elif self.info.siteUrl.find('29cm') != -1:
            self.extract_29cm_info()
        elif self.info.siteUrl.find('kream') != -1:
            self.extract_kream_info()
        # error 나는 오류 case들
        elif self.info.siteUrl.find('threetimes') != -1:
            self.extract_threetimes_info()
        else:
            self.extract_shoppingmall_info()
                

    # extract information from shopping mall     
         
    ## static page
    def extract_zigag_info(self,):
        soup = self.scraper.get_beautifulSoup()
    ...
    def extract_musinsa_info(self):
        ...
        for info_candidate in brand_info_candidate: 
            try:
                self.info.brand = driver.find_element(By.CSS_SELECTOR, info_candidate).text
                break
            except:
                self.info.brand = None
    ...
    def extract_threetimes_info(self):
        #static part
        soup = self.scraper.get_beautifulSoup()
        # 이미지
        meta_tags = soup.find_all('meta', property="og:image")
    ...
    ## extra page   
    def extract_shoppingmall_info(self):
        soup = self.scraper.get_beautifulSoup()
        ...
        if self.valid_product() == False:
            self.extract_dynamic_shoppingmall_info()


    def extract_dynamic_shoppingmall_info(self):
        ##dynamic part  
        driver = self.scraper.get_webdriver()
